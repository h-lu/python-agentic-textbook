...
> "过早优化是万恶之源。"
> — Donald Knuth，《计算机程序设计艺术》作者，图灵奖得主

近几年，DeepMind 的 AlphaGeometry 系列在国际数学奥林匹克几何题上展示了接近顶尖学生的推理能力；更早的 AlphaGo 也在围棋这种决策游戏上击败了人类冠军。这些系统有一个共同点：它们都需要根据不同情况选择下一步策略。本周，你将赋予 Python 程序做选择的能力。这不只是写几个 `if` 语句，而是让程序从"死板地执行命令"进化为"能根据情况灵活应对"的智能体。
...
7. 了解 `break` 和 `continue` 如何影响循环流程（延伸）
...
- 用 `if`/`elif`/`else` 做判断
- 用 `while` 实现“猜到为止”
- 用 `for + range()` 控制最多猜 5 次
- 用比较表达式判断“大了 / 小了 / 猜对了”

**见过即可**
- `break`：猜中后跳出循环
- `random.randint()`：生成随机答案
- `int(input())`：把用户输入从字符串转成数字
...
>
> 到今天，编辑器里的 AI 补全已经相当成熟：它可以给出下一行、下一段，甚至生成一个函数草稿。但建议越长，越需要人工审查。这和你写的 `for attempt in range(5)` 思路一致：控制节奏，一步步确认结果。
>
> 参考（访问日期：2026-02-22）：
