...
说明：本练习使用 `split(",")` 是为了练习字符串拆分。真实 CSV 可能包含引号、转义逗号和换行字段，应优先使用 Python 标准库 `csv`。
...
说明：本题的 IP 正则是“格式匹配”练习，只检查文本“像不像 IP”，不验证每一段是否在 0–255 之间。真实项目中应使用更严格的校验，或借助 `ipaddress` 模块。
- 邮箱正则（简化版）：`r"[\w.-]+@[\w.-]+\.\w+"`
- 手机号正则：`r"^1[3-9]\d{9}$"`
