...
近年的软件行业正在经历一场有趣的"回归"。几年前，微服务（microservices）架构是很多团队的答案——把系统拆成几十上百个小服务，每个服务独立部署、独立运行。但越来越多团队也意识到：过度拆分会带来网络调用、部署协调和排障成本。Amazon Prime Video 团队公开分享过把一段过度分布式的视频分析流程收敛为单体服务后显著降低成本的案例。
...
阿码在旁边举手：“等一下，这里有两个词——形参和实参，它们有什么区别？”

这里有两个容易混淆的词：

- **形参（parameter）**：定义函数时写的占位名字，例如 `def km_to_miles(km):` 里的 `km`。
- **实参（argument）**：调用函数时传入的实际值，例如 `km_to_miles(10)` 里的 `10`。

第一次学习时，你可以先记住一句话：形参像“函数入口的名字”，实参是“真正送进去的数据”。
...
>
> 近年的架构讨论也在印证这一点：一些团队开始从过度复杂的微服务回到"模块化单体"，因为**真正的模块化不在服务边界，而在代码内部**。一个写得很烂的微服务，不如一个结构清晰的单体。Amazon Prime Video 的公开案例也提醒我们：拆分不是目的，降低复杂度才是目的。
>
...
>
> 多项研究和工具报告都显示，AI 已经能参与相当一部分代码编写，但"生成出来"不等于"可以直接合并"。开发者仍然会因为 bug、可读性、上下文不匹配等原因修改或拒绝 AI 的建议。
>
