...
> "错误处理应该是系统的常态，而不是事后补丁。"
> — Joe Armstrong，Erlang 之父，容错系统先驱

你有没有经历过这种时刻：精心准备了一个 PPT,演示的时候电脑突然蓝屏；玩游戏打到关键时刻,程序直接闪退；转账填了一堆信息,点击"确认"后网页崩溃——钱扣了,账没转过去。

这些崩溃背后的罪魁祸首,往往是一个叫"未处理异常"的小东西。

2024 年 7 月 19 日,安全公司 CrowdStrike 推送的一次软件更新导致大量 Windows 设备蓝屏崩溃——航空、银行、医院等系统都受到影响。这被认为是近年来影响范围最大的 IT 中断之一。

原因不是黑客攻击,而是一个**未处理的异常**——代码读取了一个空指针,程序直接崩溃。

这件事给所有程序员上了一课：**"能跑"和"靠谱"之间,隔着异常处理这道鸿沟**。在 AI 时代,这个问题更明显了：AI 生成的代码经常能覆盖快乐路径,却容易漏掉输入校验、资源释放和异常分支。
...
>
> 多项研究和工程实践都指出,AI 生成的代码质量存在显著波动：它擅长补全常见样板,但容易遗漏错误处理、安全边界和业务上下文。不同报告的统计口径差异很大,所以本书不要求你记具体比例,而是要求你学会主动检查异常路径。
>
...
小北写完除法器后,又有了新想法："每次成功计算后,我还想记录一下日志。但这段代码应该放哪儿？放 `try` 里？还是 `except` 里？"

"都不太对,"老潘说,"Python 提供了两个'补充条款'来处理这种情况——**`else`** 和 **`finally`**。"
