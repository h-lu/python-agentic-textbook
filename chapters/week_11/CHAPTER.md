...
> "类型不是束缚，而是契约——它让团队协作成为可能。"
> — Robert C. Martin，《代码整洁之道》作者，敏捷宣言签署人

AI 编程助手已经成为许多开发者的日常工具。Stack Overflow 等开发者调查也持续追踪 AI 工具采用情况，但不同报告对"正在使用""计划使用""由 AI 生成的代码比例"口径不同，数字会快速变化。

但趋势背后隐藏着一个稳定问题：**能跑**不等于**好维护**。AI 擅长生成字典和列表的嵌套结构，但如果没有清晰的数据模型，代码会随着项目增长变得越来越难以理解。

这正是本周主题的背景意义。dataclass 和类型提示不仅让代码"自己说明自己"，更成为 AI 时代协作的"契约"——类型提示就像给 AI 的"说明书"，告诉它这里应该是什么类型的数据。
...
>
> AI 编码模型的表现高度依赖于"上下文质量"——类型提示就是提升上下文质量的关键因素之一。具体模型排名和准确率更新很快，本书不把这些动态数字当作需要记忆的知识点。
>
...
>
> 很多 AI 生成代码的问题都与**状态管理不当**有关——比如"已完成的订单被取消"、"删除后的数据还能访问"、"状态转换绕过了必要的验证"等等。具体事故率会因项目、工具和统计口径而变化，但这类风险是真实存在的。
>
