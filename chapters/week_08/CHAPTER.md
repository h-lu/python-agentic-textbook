...
AI 编程工具已经进入日常开发流程：补全、生成函数、解释报错、草拟测试都变得更快。但一个悖论随之浮现：代码写得越快，隐藏的 bug 也可能越快进入项目。AI 可以生成测试用例，但它尤其容易漏掉边界条件和复杂业务规则。当一行 AI 生成的代码能在几秒内完成，我们还需要花时间写测试吗？答案是：更需要。测试不再是"可选项"，而是 AI 时代代码质量的守门人。本周你将学习 pytest——Python 最流行的测试框架，掌握让代码值得信赖的核心技能。
...
>
> 很多团队已经开始用 AI 生成测试骨架、补充断言、解释失败原因。这听起来很美好——但有一个陷阱：AI 在边界条件和复杂业务逻辑上表现不稳定。它擅长生成"正常路径"的测试，却容易遗漏那些真正会出问题的边缘情况。
>
...
>
> 在 AI 生成代码的时代，TDD 的价值更加明显：测试成为人类意图与 AI 实现之间的桥梁。没有测试，你只是在"希望" AI 理解对了；有了测试，你能"确认" AI 理解对了。
>
