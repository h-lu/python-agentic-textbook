...
Python 长期活跃在数据处理、机器学习和 AI 工程领域。为什么？因为 Python 的列表（list）、字典（dict）这些"容器"，和真实项目里组织数据的方式高度契合。AI 不是魔法——它是数据驱动的，而你本周要学的，正是"如何组织数据"。
...
>
> 但这里有个反直觉的事实：Python 原生列表并不是 AI 领域的主力。真正做大规模数值计算时，常用的是 **NumPy 数组**、PyTorch/TensorFlow 张量等结构。它们使用更紧凑的内存布局和底层优化，适合批量计算；Python 列表则更适合入门理解和一般脚本处理。
>
> 在 AI/ML 工程里，Python 是最常见的入口语言之一。他们不用原生列表做大规模计算，而是用 NumPy 数组和 PyTorch/TensorFlow 张量。但你今天学的列表操作——`append()`、索引访问、切片——是理解这些高级工具的基础。
>
...
这一节你见到了很多列表方法。别急着全部背下来。本周真正需要熟练的是三件事：

1. 用列表装一组数据；
2. 用 `for` 遍历它；
3. 知道添加用 `append()`，排序时优先分清 `sort()` 和 `sorted()`。
