...
export default async function Week8Page() {
  const contentPath = path.join(process.cwd(), 'public', 'chapters', 'week-08.md')
  ...
  const currentIndex = data.chapters.findIndex((c: any) => c.week === '08')
    ...
    <div className="min-h-screen bg-white">
      <header className="bg-slate-900 text-white py-12">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <h1 className="text-3xl md:text-4xl font-bold">{data.chapters[currentIndex]?.title || 'Week 8'}</h1>
        </div>
      </header>
